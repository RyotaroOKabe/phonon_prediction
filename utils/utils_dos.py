...
from utils.utils_plot import loss_distx
...
def dos2cv(g, omega, astruct, T_lst=[293.15]):
    # iinit
    C_v_kg = []
    # omega unit to Hz
    # omega_hz = np.array(omega[1:])*const.c*100*2*np.pi # wavenumber to circular frequency
    omega_hz = np.array(omega)*const.c*100*2*np.pi # wavenumber to circular frequency
    # process g(omega)
    g_norm = np.array(g)/np.trapz(np.array(g), omega_hz)
    unique_symbols = set(astruct.get_chemical_symbols())
    num_species = len(unique_symbols)
    num_sites = len(astruct.positions)
    if num_species == 1:
        g_norm_xSitesNum = 3*g_norm
    else:
        g_norm_xSitesNum = 3*num_sites*g_norm   
    # Calculate Cv(T) for T in T_lst
    for T in T_lst:
        ...
        C_v_kg.append(C_v_uc*1000/(sum(atom.mass for atom in astruct)*1e-24))
    return np.array(C_v_kg)


def plot_cv(df_in, T_lst, data, header, fnum=100, title=None, n=5, m=1, lwidth=0.5, windowsize=(2.5, 2.2), palette=palette, formula=True, gtruth=True):
    """_summary_
    ...
    for k in range(num*m*n):
        ...
        rband = ds.iloc[i]['real_band']
        pband = ds.iloc[i]['output_test']
        astruct = data[data['id']==ds.iloc[i]['id']]['structure'].item()
        fpmax, fpmin = np.max(pband), np.min(pband)
        frmax, frmin = np.max(rband), np.min(rband)
        fmax, fmin = max(frmax, fpmax), min(frmin, fpmin)
        frange = fmax-fmin
        ...
        pr_dos = dos_from_band(pband, x)
        re_dos = dos_from_band(rband, x)
        pr_cv = dos2cv(pr_dos, x, astruct, T_lst)
        re_cv = dos2cv(re_dos, x, astruct, T_lst)
        if gtruth:
            ax.plot(T_lst, re_cv, color='k', linewidth=lwidth*0.7)
        ax.plot(T_lst, pr_cv, color=cols[k], linewidth=lwidth)
        id_list.append(ds.iloc[i]['id'])
    ...
    fig.patch.set_facecolor('white')
    if title: fig.suptitle(title, ha='center', y=1., fontsize=fontsize)
    fig.savefig(f"{header}_{title}_Cv.png")
    fig.savefig(f"{header}_{title}_Cv.pdf")
    print(id_list)